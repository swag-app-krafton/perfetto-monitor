...
- **When this was written.** 2026-09-24, about 13:30 IST, from the history up to `8b27552` and the working tree at that time. Other sessions were still editing the iOS and stability work, so for those two sections check `git status` before relying on the details.
...
| **F-014** 🚧 | **Stability.** Hangs, JS errors with stacks resolved against the build's source map, and crashes, on both platforms. F-028 (committed `a3428b4`…`4e7ff5c`) adds ANRs and every crash with its crash log, and splits the screen: Crashes & ANRs for JS exceptions, ANRs and crashes, and hangs on Frame pacing. | [Section 15](#15-crashes--anrs-and-hangs-on-frame-pacing-f-014-f-028) |
...
|---|---|---|---|---|
| **Analyse a trace** ✅ | Extracts metrics from a `.pftrace`, records the run, works out regressions and gives a verdict. | `swagperf analyse <trace>` with `--app`, `--derive`, `--path-kind`, `--label`, `--git-sha`, `--app-version`, `--device`, `--no-llm`, `--backend auto\|cli\|api\|heuristic`, `--json`, `--fail-on never\|fail\|warn`. Exit code 1 on a fail verdict. Exit code 2 when an app marked instrumented has no `step:` markers. | `cli.py`, `extract.extract_any()`, `store.record()` | `bebc974` |
...
✅ `c5132b9` through `6c6e4b2`, and the React page in `d86cacf`. Device: none, it reads a trace.
...
- **`swagperf reset [--yes] [--keep-traces]`** ✅ `8486bff`:
...
- **Design system** ✅ `cd429fd`, `6c2d9d4`, `1be77cc`. It lives in `frontend/src/design/`: foundations (colour tokens per theme), primitives, components, charts and hooks. Pages import only from `@/design`.
...
✅ `47402c0` (F-006; its code is committed, but it has not yet been run end to end on a phone). Needs an Android phone, Node, and `npm ci --prefix flashlight`.
...
The experiment behind the "never at the same time" rule (`69a7a4b`, `experiments/flashlight-concurrency/`, `docs/flashlight-perfetto-observations.html`) found that once Flashlight starts, Perfetto loses its scheduler data, slices, markers and RAM samples for good, and neither tool logs an error.
...
- **`./perfetto_init`** ✅ `e61ef33`, with `./perfetto` as an alias:
...
- **Run skill** ✅ `b6e429d`. `.claude/skills/run-perfetto-monitor/driver.py`:
...
- **Graphify** ✅ `c9c95e6`. A query-first hook for Cursor, and the committed knowledge graph in `graphify-out/`.
...
F-014 is committed in `bacfa81`. F-028 is committed in `a3428b4`…`4e7ff5c` (2026-09-25); its phone check (a real ANR and crashes on the V2514) is still open. Design: `docs/superpowers/specs/2026-09-25-crashes-anrs-tab-design.md`.
...
- **NLP mobile automation in CI** 📝 `0c4ff40`. [Plan](nlp-mobile-automation-ci-plan.html) and [ADR 0001](decisions/0001-nlp-mobile-automation-ci.md) (proposed).
...
✅ `e428d0b` (2026-09-25). Device: none.
...
| Commit | Date | Change |
|---|---|---|
| `bebc974` | 09-22 | Perfetto regression monitor for the Swag Pay shell: extraction, history, analyst, North Star targets, CLI, first dashboard, made-up traces, tests |
| `5696572` | 09-22 | Run comparison and pinned benchmark runs |
| `15c340c` | 09-22 | Any Android app through derived steps, plus a competitor catalogue |
| `71c64fa` | 09-22 | Stop tracking `apps.local.json` |
| `f63dfac` | 09-22 | Capture from the dashboard, app scoping, real-device capture fixes |
| `3835893` | 09-22 | Stress tests |
| `c5132b9` | 09-22 | Manual tracing and per-screen CPU and RAM usage |
| `e61ef33`, `52cb95f` | 09-22 | `perfetto_init` launcher and the `perfetto` alias |
| `718bd3d`…`a9ea6f1` | 09-22 | Screens fixes, screen kinds, navigation stack, live markers endpoint, "RAM usage" wording, token monitor |
| `814c2d5` | 09-23 | Device-trace accuracy: RAM usage and frames scoped to the app, honest verdicts |
| `3e58765` | 09-23 | Manual trace config slimmed from about 230 to about 53 MB per minute |
| `6c6e4b2` | 09-23 | Screens: session timeline, per-screen app jank, transition costs, cache |
| `94b32f5` | 09-23 | Live markers read incrementally |
| `47d3379` | 09-23 | Device-shaped made-up traces; no borrowing another app's markers |
| `cd429fd`…`d34c8db` | 09-23 | Dashboard rebuilt in React + TypeScript, every page |
| `23d3db2`, `6c2d9d4` | 09-23 | Copilot engine, API and panel; the design system |
| `901dc84` | 09-23 | Old dashboard removed; static paths confined to the build |
| `0c4ff40` | 09-23 | NLP mobile automation in CI: plan and ADR 0001 |
| `9935b92` | 09-23 | README: quick start and what to watch on each page |
| `4a5100d` | 09-23 | Each run records its device, device state and app build |
| `87ec1eb` | 09-23 | One run in view: Run picker, run box, Run details |
| `8486bff` | 09-23 | Reset command, Version filter, sortable Run list |
| `736a59d` | 09-23 | Tracker, product-manager agent and automatic run review |
| `69a7a4b` | 09-23 | Flashlight and Perfetto on one device: tested and ruled out |
| `47402c0` | 09-23 | Flashlight lane, the one-profiler guard, step and thread descriptions |
| `1be77cc` | 09-24 | Pages built only from the design system (T-003) |
| `b6e429d` | 09-24 | Run skill |
| `8b27552` | 09-24 | Maestro screenshot testing parked; one product-manager skill for every agent |
