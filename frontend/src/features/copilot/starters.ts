...
/** Suggested questions for a tab, phrased against the runs actually in
 *  scope, and only ones the Copilot can answer from them. */
export function startersFor(tab: string, scope: Scope | null, compare?: { a: number | null; b: number | null }): string[] {
  ...
  if (!latest) return ['What can you answer?']
  const worst = stepDeltas(latest, scope.allRuns, scope.benchmarkRun)
    .filter((d) => d.deltaMs != null && d.deltaMs >= 5)
    .sort((x, y) => y.deltaMs! - x.deltaMs!)[0]
  const base = scope.benchmarkRun ? 'the benchmark' : 'recent runs'
  const why = 'Why did TTID regress in the latest run?'
  ...
  const prior = scope.allRuns.filter((r) => r.id < latest.id).at(-1)
  const a = compare?.a ?? latest.id
  ...
  const byTab: Record<string, string[]> = {
    ...
    frames: [`What is causing slow frames in run #${latest.id}?`, 'Is the thermal drift real or environmental?'],
    ...
    manual: [`What is causing slow frames in run #${latest.id}?`, 'Summarise this session for a PR comment'],
