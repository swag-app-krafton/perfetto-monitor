...
export interface StackRow {
  ...
  label: string
  bold?: boolean
  /** Drawn faded: a reference row (the previous run) under the one in view. */
  dim?: boolean
  /** `text` is printed inside the segment at size lg, where it fits. */
  segments: { key: string; value: number; title: string; text?: string }[]
 ...
 *  Size md is a compact row per run; lg is a hero bar with each segment's
 *  value printed inside it. */
export function StackedBars({
  ...
  const lg = size === 'lg'
  return (
    <div className={lg ? s.lg : undefined}>
      <div className={s.rows}>
        {rows.map((r) => (
          <div key={r.id} className={r.dim ? `${s.grid} ${s.dim}` : s.grid}>
            ...
            <div className={s.track}>
              {r.segments.map((sg, i) => (
                <div key={i} title={sg.title} className={s.segment} style={{ width: pct(sg.value), background: colors[sg.key] ?? 'var(--tx3)' }}>
                  {lg && sg.text && sg.value / max > 0.08 && <span className={s.segmentText}>{sg.text}</span>}
                </div>
              ))}
